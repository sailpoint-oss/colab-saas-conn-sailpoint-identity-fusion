...
export class ReportEmail implements TestWorkflowRequestBeta {
    ...
    constructor(analyses: AccountAnalysis[], attributes: string[], recipient: IdentityDocument) {
        ...
        for (const analysis of analyses) {
            ...
            const record = [nativeIdentity, name, sourceName, ...attributeValues, result].map(str => `<td style="padding: 12px 15px;text-align: left;border-bottom: 1px solid #ddd;">${str}</td>`).join()
